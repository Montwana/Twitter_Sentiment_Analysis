...
lottie_confetti = load_lottieurl("https://assets3.lottiefiles.com/packages/lf20_jbrw3hcz.json")  # Positive
lottie_neutral = load_lottieurl("https://assets2.lottiefiles.com/packages/lf20_tll0j4bb.json")  # Neutral
lottie_rain = load_lottieurl("https://assets9.lottiefiles.com/packages/lf20_u4yrau.json")  # Negative
...
def plot_sentiment_summary_plotly(sentiments, title="Sentiment Summary"):
...
if option == "Type a Tweet":
    ...
    if st.button("Analyze Sentiment"):
        if st.session_state.user_input.strip() == "":
            ...
            # Reset session for this analysis
            st.session_state["typed_tweets"] = [st.session_state.user_input]
            st.session_state["typed_sentiments"] = [sentiment]
            
            st.subheader("Typed Tweets Sentiment Summary")
            plot_sentiment_summary_plotly(st.session_state["typed_sentiments"], "Typed Tweets Sentiment Summary")
            
            st.subheader("Word Cloud of Typed Tweets")
            generate_wordcloud(st.session_state["typed_tweets"])


# Fetch User Tweets (v2) with caching
        ...
        if username.strip() == "":
            ...
            with st.spinner(f"Fetching {num_tweets} tweets from @{username}..."):
                ...
                if error:
                    ...
                    # Reset session for fetched batch
                    ...
                    plot_sentiment_summary_plotly(st.session_state["fetched_sentiments"], f"Fetched Tweets Sentiment Summary (@{username})")
